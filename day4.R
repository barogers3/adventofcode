...
test <- tribble(~V1,
                ...
                "oiii ioii iioi iiio" )
library(stringr)
part1 <- as.data.frame(str_split_fixed(test$V1," ",20))

part1$dups <- apply(part1, 1, function(i) any(duplicated(i, incomparables = c(""," "))))
...
#test2 <- as.data.frame(str_split_fixed(test$V1," ",20))
fl<-data.frame()
rownum<-1
for (i in 1:nrow(test)) {
  test2 <- str_split_fixed(test$V1[i]," ",20)
  for (k in 1:length(test2)) {
    if (test2[k] != "") {
      ana <- anagram(test2[k], words = test2)
      if (length(ana) > 1) {
        rownum <- i
      }
    }
...
length(unique(fl$X1))-1

#need to get row numbers of valid ones from first one
valid1 <- part1 %>% mutate(rownum = row_number()) %>% filter(dups == FALSE)

#remove row numbers that are invalid from part 2
validpart2 <- valid1 %>% anti_join(distinct(fl), by = c("rownum" = "X1"))
